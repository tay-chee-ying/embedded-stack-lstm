...
=======
print("START")
>>>>>>> 687d97fc2d65de4dc11c30e1897fa03412acc36f
import torch
...
learning_rate = 0.001
epoch_num = 10
test_embedded_size = 60
test_stack_size = 60
vocab_size = 7
print("--------------") 
train_name = "/content/drive/My Drive/estack_overdrive/datasets/wcw2_l_training_dataset.npz"
test_name = "/content/drive/My Drive/estack_overdrive/datasets/wcw2_l_testing_dataset.npz" 
save_prefix = "/content/drive/My Drive/estack_overdrive/save_{}/".format(system_number)
#print to make sure
print("System number: {} Random seed: {}".format(system_number,random_seed))
...
print("learning_rate: {}, epoch_num: {}, epsilon: {}, init_temp: {}".format(learning_rate,epoch_num,epsilon,temp))
=======
torch.set_default_tensor_type('torch.DoubleTensor')
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
...
input_list=training_dset['input']
...
optimizer = torch.optim.Adam(estack_lstm.parameters(),lr = learning_rate)
accumulated_accuracies,accumulated_running_average,accumulated_errors= [],[],[]
running_average,global_counter = 0,0
...
for e_c in range(epoch_num):
    for c in range(len(input_list)):
        ...
        optimizer.zero_grad()
        lstm_hidden,selector,embedded_stack = estack_lstm.init_hidden()
        dummy_output = torch.zeros([len(input_list[c]),vocab_size]).to(device)
        partial0 = 0
        #evaluate model
        for c1 in range(len(input_list[c])):
                temp = np.maximum(temp * np.exp(-ANNEAL_RATE), temp_min)
                input_symbol=torch.from_numpy(input_list[c][c1]).to(device)
                input_symbol=input_symbol.view(1,1,-1)
                output,lstm_hidden,selector,embedded_stack,debug_tuple=estack_lstm(
                        input_symbol,lstm_hidden,selector,embedded_stack,temp)
                dummy_output[c1,:]=output.view(-1) 
        ...
        accumulated_accuracies += [1-partial0/len(input_list[c])]
        running_average += 1-partial0/len(input_list[c])
        accumulated_errors += [loss.cpu().detach().numpy()]
        if global_counter%1000==0:
            print("Epoch: {} GC: {} Temp: {} MSE Loss: {} Partial Accur: {} Time: {}".format(e_c, global_counter,temp,loss,running_average/100,time.time() - start_time))
            start_time = time.time()
        if global_counter%100==0:
            accumulated_running_average+=[running_average/100]
            running_average=0
        ...
        if global_counter%500==0:
            ...
            torch.save(estack_lstm.state_dict(), save_prefix+"/in_train_models/estack_{}.pt".format(global_counter))
        global_counter+=1
#perform testing
...
<<<<<<< HEAD
            lstm_hidden,head,embedded_stack=estack_lstm.init_hidden()
=======
            lstm_hidden,embedded_stack=estack_lstm.init_hidden()
>>>>>>> 687d97fc2d65de4dc11c30e1897fa03412acc36f
            ...
            for c1 in range(sample_length):
                ...
                input_symbol=input_symbol.view(1,1,-1)
<<<<<<< HEAD
                output,lstm_hidden,head,embedded_stack,debug=estack_lstm(
                        input_symbol,lstm_hidden,head,embedded_stack,0.5)
=======
